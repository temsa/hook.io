  ...
  return {
    query: function (params, callback) {
      ...
      if(name) {
        ...
        var sessions = Object.keys(self.server.proto.sessions).map(function(key){
          return self.server.proto.sessions[key];
        });
        ...
        async.map(sessions,
          ...
          function mapAllResultsWithEvent(err, resultList) {
            ...
            var details = resultList
              .filter(function onlyWithEvent(result){
                return result.hasEvent;
              })
              .map(function getDetails(result){
                return self._names[result.name];
              });
          ...
          self.toIPs(host, function onIP(err, hosts) {
            var details = Object.keys(self._names).map(function getHooks(key) {
              var hook = self._names[key];
              if(hosts.some(function hasHost(host){
                return (host === hook.remote.host) || isWildcardIP(hook.remote.host);
              })) {
                return hook;
              }
            }).filter(function(detail){
            ...
            if(details.length>0) return callback(null, details);
            return callback(new Error("No hook for host "+host+" is connected (anymore?)"),[]);
          });
      }
    }
  };
