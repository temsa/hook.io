...
var Hook = exports.Hook = function (options) {
  ...
  this.defaults['hook-host']   = options['hook-host']   || '0.0.0.0';
  ...
  if(self['hook-message-debug']) {
    self.onAny(function(data) {console.log( self.server ? self.name.red : self.name.cyan, '<-'.blue, '[', self.event.bold, ']', 'data'.green, data)});
  }
};
...
Hook.prototype.query     = require('./query')(DELIMITER).query;
...
//
Hook.prototype.emit = function (event, data, callback, group) {
  var self = this;
  //
  ...
  if(self['hook-message-debug']) {
    console.log( self.server ? self.name.red : self.name.cyan, '-> '.blue.bold, '[', event.bold, ']', 'data'.green.bold, data, 'group'.yellow.bold, group);
  }

  if (group !== true && self.remote) {
    //
    ...
    //
    this.remote.message(this.name + DELIMITER + event, data, callback, group);
  }
  ...
  //
  return EventEmitter.prototype.emit.apply(this, [event, data, callback, group]);
}
  ...
  self.toIPs(self['hook-host'], function onResolve(err, hosts){
    ...
    self.server = dnode(function (client, conn) {
      ...
      function removeSelf() {
        ...
        if(self.server && self.server.proto.sessions[conn.id]) {
          self.server.proto.sessions[conn.id].end();
          delete self.server.proto.sessions[conn.id];
        }
      };

      conn.on('end', removeSelf);
      conn.on('error', removeSelf);
      conn.on('close', removeSelf);
      ...
      this.report = function (name, type, reported) {
        ...
        //
        // Store server details in name as well
        //
      ...
      //
      self.onAny(function (data, callback, remote) {
        // Client is currently connecting and has not reported yet, we can't send him anything just yet.
        if(!Object.keys(client).length) return;
        
        var parts = self.event.split(DELIMITER),
            event = !remote ? [self.name, self.event].join(DELIMITER) : self.event;

        if(!client.message || (parts[0] === client.name)) return;
    ...
    self.server.on('ready', function (err) {
      self.on("*::query", self.query.bind(self));
      self.on("query", self.query.bind(self));
...
Hook.prototype.connect = function (options, callback) {
  ...
  client = dnode({
    message: function (event, data, callback) {
      if(self['hook-message-debug']) {
        console.log( self.name.green, 'message'.magenta, '[', self.event.bold, ']', 'data'.green, data);
      }
