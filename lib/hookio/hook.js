...
Hook.prototype.emit = function (event, data, callback) {
  ...
  if(self['event-debug'] === true) {
    console.log( self.server ? self.name.red : self.name.cyan, '->'.green.bold, '*'.yellow.bold, '[', event.bold, ']', 'data'.green, data);
  }
...
Hook.prototype.listen = function (options, callback) { 
  ...
  self.toIPs(self['hook-host'], function onResolve(err, hosts){
    ...
    var client; //store the client for easy acces to it
    self.server = dnode(function (_client, conn) {
      client = _client;
      ...
      this.message = function (event, data, callback) {
        if(self['message-debug'] === true) {
          var type ='<>';
          
          if(self.hasEvent(event, true)) {
            type = '<='.green.bold ;
          }
          console.log( self.name.red.bold, type, client.name.cyan.bold,'[', event.bold, ']', 'data'.green.bold, data);
        }
      ...
      self.onAny(function (data, callback) {
        ...
        if(self['event-debug'] === true) {
          ...
          console.log( self.name.red, type, client.name.cyan,'[', event.bold, ']', 'data'.green.bold, data);
        }
        ...
        if (client.message && (parts[0] !== client.name)) {
          ...
          client.hasEvent(parts, callback, function(err, send) {
            ...
            if(self['message-debug'] === true) {
               var origin = event.split(DELIMITER)[0];
               console.log( self.name.red.bold, '=>'.bold, client.name.cyan.bold, '[', event.bold, ']', 'data'.green.bold, data);
            }
...
Hook.prototype.connect = function (options, callback) {
  ...
  client = dnode({
    message: function (event, data, callback) {
      ...
      if(self['message-debug'] === true) {
         var origin = event.split(DELIMITER)[0];
         console.log( self.name.cyan.bold, '<='.bold, origin.red.bold, '[', event.bold, ']', 'data'.green.bold, data);
      }
      self.emit(event, data, callback);
    },
  ...
  self.onAny(function (data, callback) {
    ...
    if(self['event-debug'] === true ) {
      ...
      console.log( self.name.cyan, type, (client.name || '<parent>').red,'[', event.bold, ']', 'data'.green.bold, data);
    }
  });
};
