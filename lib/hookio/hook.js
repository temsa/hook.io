...
Hook.prototype.listen = function (options, callback) { 
  ...
  this.server.on('ready', function (err) {
    self.emit('hook::listening', self['hook-server'] + ':' + self['hook-port']);
    self.emit('hook::ready', self['hook-server'] + ':' +self['hook-port']);
  ...
  //
  try {
    this.server.listen(self['hook-port'], self['hook-host']);
  }
  catch (ex) {
    if (callback) {
      return callback(ex);
