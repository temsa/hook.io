    ...
    function onDetails(options) {
        ...
        if(name) {
          ...
          /*var details = Object.keys(self._names)
            .map(function(key){ if(self._names[key].type === type) return self._names[key]; })
            .filter(function(detail){return detail});*/
          //async.map(Object.keys(self._names), function(hookName) {
          ...
          /*if(details.length>0) {
            ...
            callback(new Error("No hook listening for `"+event+"` is connected (anymore?)"),[]);
          }*/
console.log('self'.red); require("eyes").inspect(self);
          
          var sessions = Object.keys(self.server.proto.sessions).map(function(key){return self.server.proto.sessions[key]});
          
//          session.remote.hasEvent(event.split(DELIMITER), session.remote, console.log);
          async.map(sessions,
